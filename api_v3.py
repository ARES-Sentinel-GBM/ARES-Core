...
class FBARequest(BaseModel):
    warburg_factor: float = Field(1.0, ge=0.1, le=5.0)
    ko_threshold:   float = Field(0.05, ge=0, le=1.0)

@app.post("/fba/solve", tags=["FBA Metabolism"])
async def fba_solve(req: FBARequest, _=Depends(verify_token)):
    """Risolve FBA GBM: max biomassa s.t. S·v=0, lb≤v≤ub."""
    from modules.fba_metabolism import GBMMetabolicModel
    model = GBMMetabolicModel(req.warburg_factor)
    res   = model.solve_fba()
    return {
        "status":        res["status"],
        "biomass_flux":  res["biomass"],
        "sv_max":        res["sv_max"],
        "active_fluxes": res["active"],
        "warburg_factor": req.warburg_factor,
        "disclaimer": "Computational model. Validate experimentally.",
    }

@app.post("/fba/essentiality", tags=["FBA Metabolism"])
async def fba_essentiality(req: FBARequest, _=Depends(verify_token)):
    """Gene essentiality via knockout sistematico nel modello FBA GBM."""
    from modules.fba_metabolism import GBMMetabolicModel
    model = GBMMetabolicModel(req.warburg_factor)
    ge    = model.gene_essentiality(threshold=req.ko_threshold)
    return {
        "n_genes":    len(ge),
        "n_essential": int(ge["essential"].sum()),
        "genes":      ge.to_dict(orient="records"),
    }

@app.get("/fba/warburg-scan", tags=["FBA Metabolism"])
async def fba_warburg(_=Depends(verify_token)):
    """Scansione effetto Warburg: biomassa vs intensità glicolitica."""
    from modules.fba_metabolism import GBMMetabolicModel
    model = GBMMetabolicModel()
    ws    = model.warburg_scan()
    return ws.to_dict(orient="records")
